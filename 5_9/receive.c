...
void *sendth(void *args){
	...
	if ((qd = mq_open(QNAME2, O_CREAT | O_RDWR, 0600, &q_attr)) == -1) {perror("mq_open failed");exit(1);}
	while(1){
		scanf("%s", send_data);
...
void *receiveth(void *args){
	...
	if ((qd = mq_open(QNAME1, O_RDWR | O_NONBLOCK,0600, NULL)) == -1) {perror("mq_open failed");exit(1);}
	q_attr.mq_flags = 0; 
